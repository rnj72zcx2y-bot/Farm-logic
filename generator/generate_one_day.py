...
def generate_candidate(date_utc: str, difficulty: str, attempt: int) -> Dict[str, Any]:
    """TEMP generator: produces a simple connected, domino-tileable shape.

    This is only to enable developing and testing the solver + pipeline.
    Replace later with your real generator (random shapes, regions, rules, cards).
    """
    ...
    for r in range(rows):
        for c in range(cols):
            if (r, c) not in active_set:
                blocked.append({"cell": [r, c], "type": blocker_types[k % 3]})
                k += 1

    # TEMP cards: correct count, neutral content
    ...
    # TEMP regions: empty for Stage-1
    return {
        "dateUtc": date_utc,
